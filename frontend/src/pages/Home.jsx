...
const FORMAT_CONFIG = {
  league: {
    title: 'StreetBash League',
    accent: 'text-cricket-accent',
    buttonLabel: 'Launch League Event',
    buttonClass: 'bg-cricket-lightGreen hover:bg-green-600',
    navigateTo: (isAdmin) => isAdmin ? '/admin/teams' : '/fixtures',
    emoji: '🏆'
  },
  knockout: {
    title: 'Knockout Arena',
    accent: 'text-orange-400',
    buttonLabel: 'Launch Knockout Event',
    buttonClass: 'bg-orange-600 hover:bg-orange-500',
    navigateTo: (isAdmin) => isAdmin ? '/admin/teams' : '/fixtures',
    emoji: '⚔️'
  },
  test: {
    title: 'Test Match Center',
    accent: 'text-blue-400',
    buttonLabel: 'Launch Test Event',
...
export default function Home({ isAdminMode = false }) {
  ...
  const config = FORMAT_CONFIG[activeFormat] || FORMAT_CONFIG.league;

  useEffect(() => {
    const formatQuery = activeFormat ? `?format=${activeFormat}` : '';
    fetch(`${API_BASE}/tournament/list${formatQuery}`)
      .then(res => res.json())
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/tournament/create`, {
        ...
        body: JSON.stringify({ ...form, format: activeFormat })
      });
  ...
  const selectTournament = (id) => {
    ...
    navigate(config.navigateTo(isAdminMode));
  };

  const formatBadge = (format) => {
    const badges = {
      league: { label: 'League', cls: 'bg-green-900 text-green-400 border-green-700' },
      knockout: { label: 'Knockout', cls: 'bg-orange-900 text-orange-400 border-orange-700' },
      test: { label: 'Test', cls: 'bg-blue-900 text-blue-400 border-blue-700' }
    };
    const b = badges[format] || badges.league;
    return <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded border ${b.cls}`}>{b.label}</span>;
  };
    ...
    <div className="flex flex-col items-center justify-center min-h-[70vh] text-center space-y-10 px-4">
      <div className="space-y-4 max-w-2xl animate-fade-in relative pt-10">
        ...
        <h1 className="text-5xl md:text-7xl font-black text-white uppercase tracking-tighter drop-shadow-2xl">
          {config.emoji} <span className={config.accent}>{config.title}</span>
        </h1>
        ...
        <Link to="/" className="inline-block text-xs text-gray-500 hover:text-white uppercase tracking-widest font-bold border border-gray-700 px-4 py-1.5 rounded-full hover:bg-gray-800 transition mt-2">
          ← Change Format
        </Link>
      </div>
         ...
         <div className={`grid grid-cols-1 ${isAdminMode ? 'md:grid-cols-2' : ''} gap-8`}>
            ...
            {isAdminMode && (
               <div className="bg-black/60 p-6 rounded-xl border border-gray-700 shadow-[0_0_20px_rgba(34,197,94,0.1)]">
                  ...
                  <form onSubmit={handleCreate} className="space-y-4 text-left">
                     ...
                     <button type="submit" className={`w-full ${config.buttonClass} text-white font-bold py-3 rounded-lg uppercase tracking-wider transition shadow-lg`}>{config.buttonLabel}</button>
                  </form>
               </div>
            )}
            ...
            <div className="bg-black/60 p-6 rounded-xl border border-gray-700 shadow-inner flex flex-col pt-6">
               ...
               <div className="space-y-3 overflow-y-auto flex-1 max-h-64 pr-2 custom-scrollbar">
                  {tournaments.length === 0 && <p className="text-gray-500 italic text-sm p-4">No {activeFormat} tournaments found. Create your first season!</p>}
                  {tournaments.map(t => (
